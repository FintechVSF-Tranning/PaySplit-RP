...
DO $$ BEGIN
    ...
    CREATE TYPE debt_status      AS ENUM ('awaiting','pending_confirmation','stalled_confirmation','rejected','settled');
EXCEPTION WHEN duplicate_object THEN null;
END $$;
ALTER TYPE debt_status ADD VALUE IF NOT EXISTS 'rejected' BEFORE 'settled';
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS bill_item_assignments (
    ...
    group_id      UUID NOT NULL,
    member_id     UUID NOT NULL, 									-- Ép buộc NOT NULL. Nếu Creditor tự chịu, gán thẳng ID của Creditor vào đây.
...
-- Danh sách thành viên và trọng số ở trạng thái DRAFT, dùng cho chia toàn bộ
-- hóa đơn (equal/percentage) kể cả khi hóa đơn không có line item.
...
CREATE INDEX IF NOT EXISTS idx_bill_participants_member ON bill_participants(member_id);
...
CREATE TABLE IF NOT EXISTS ledger_transactions (
    ...
    reversal_of_transaction_id  UUID REFERENCES ledger_transactions(id),    -- Dùng khi có sai sót cần hủy một giao dịch cũ.
...
CREATE TABLE IF NOT EXISTS ledger_postings (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    FOREIGN KEY (transaction_id, group_id) REFERENCES ledger_transactions(id, group_id) ON DELETE CASCADE,
    FOREIGN KEY (member_id, group_id) REFERENCES group_members(id, group_id)
);
...
CREATE TABLE IF NOT EXISTS debts (
    ...
    UNIQUE (id, group_id),
    FOREIGN KEY (bill_id, group_id) REFERENCES bills(id, group_id),
    FOREIGN KEY (debtor_member_id, group_id) REFERENCES group_members(id, group_id),
    FOREIGN KEY (creditor_member_id, group_id) REFERENCES group_members(id, group_id)
);

-- Nguồn nghĩa vụ của một khoản nợ đã được Settlement Controller đơn giản hóa.
-- Tổng amount theo debt_id phải bằng debts.amount và được kiểm tra trong cùng
-- transaction bởi application/service khi rebuild các debt còn awaiting.
CREATE TABLE IF NOT EXISTS debt_sources (
    debt_id     UUID NOT NULL,
    bill_id     UUID NOT NULL,
    group_id    UUID NOT NULL,
    amount      BIGINT NOT NULL CHECK (amount > 0),
    PRIMARY KEY (debt_id, bill_id),
    FOREIGN KEY (debt_id, group_id) REFERENCES debts(id, group_id) ON DELETE CASCADE,
    FOREIGN KEY (bill_id, group_id) REFERENCES bills(id, group_id)
);
...
DROP INDEX IF EXISTS idx_debts_unsettled;
CREATE INDEX idx_debts_unsettled ON debts(debtor_member_id) WHERE status <> 'settled';
